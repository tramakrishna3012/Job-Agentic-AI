...
- Set a monthly spend cap in the OpenAI dashboard / monitor Google AI Studio quota as a safety net against runaway calls (e.g. a retry loop gone wrong).

## 4. Twilio / WhatsApp / Notifications
- Sandbox credentials and webhook URLs are sensitive — treat them with the same care as production keys.
...
- If any key leaks (e.g. accidentally committed): rotate it immediately — OpenAI, Google AI Studio, Twilio, and Google Cloud all support key/token revocation from their dashboards — then scrub git history if it was committed (`git filter-repo` or BFG Repo-Cleaner).
